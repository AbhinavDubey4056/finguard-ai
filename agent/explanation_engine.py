...
def generate_explanation(verdict: str, confidence: float, risk_level: str) -> str:
    ...
    if verdict == "DEEPFAKE":
        ...
        base_msg = f"The analysis suggests the video is authentic (Confidence: {100 - percentage}% real)."
    else:
        base_msg = "The analysis resulted in an ambiguous verdict."

    # Add risk-specific context
    if risk_level == "CRITICAL":
        risk_msg = " This video shows high-probability synthesis and should be treated as high-risk content."
    elif risk_level == "HIGH":
        risk_msg = " Significant anomalies were found. Manual verification is strongly recommended."
    elif risk_level == "MEDIUM":
        risk_msg = " The model encountered conflicting patterns. Please review the frame-by-frame details."
    else:
        risk_msg = " No significant anomalies detected."

    return f"{base_msg}{risk_msg}"

# Overloaded version for main.py pipeline compatibility
def generate_explanation_from_decision(score: float, decision: Dict[str, Any]) -> str:
    """
    Helper function to match the call signature used in app/main.py.
    """
